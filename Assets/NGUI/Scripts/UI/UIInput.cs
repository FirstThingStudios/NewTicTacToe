...
public class UIInput : MonoBehaviour
{
	...
	//Created by Samuel
	protected bool callInsertFromUpdate = true;
	...
	void OnGUI ()
	{
        if (isSelected)
        {
            ...
            if (Event.current.rawType == EventType.MouseDown)
                ProcessEventMouse(Event.current);
            if (Event.current.rawType == EventType.ScrollWheel)
                ProcessEventMouseWheel(Event.current);
        }
	}
    ...
    /// <summary>
    /// Created by Samuel on 16/11/2014 for Keyboard Option purposes.
    /// overrided by UIInputKeyBind, called in OnGUI()
    /// </summary>

    protected virtual bool ProcessEventMouseWheel(Event ev)
    {
        return false;
    }
    ...
    protected virtual bool ProcessEventMouse (Event ev)
    {
        return false;
    }
